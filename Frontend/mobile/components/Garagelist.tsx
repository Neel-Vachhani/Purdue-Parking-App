  ...
  type DistanceOption = { label: string; value: number | null };
    ...
    type AvailabilityOption = { label: string; value: number };
    ...
    //use origin address
    type OriginCoords = { lat: number; lng: number } | null;

    async function geocodeOriginAddress(address: string): Promise<OriginCoords> {
      if (!address.trim()) return null;
      try {
        const encoded = encodeURIComponent(address.trim());
        const res = await fetch(
          `https://nominatim.openstreetmap.org/search?q=${encoded}&format=json&limit=1`,
          { headers: { "User-Agent": "PurdueParking/1.0" } }
        );
        ...
        console.warn("Geocoding failed for origin address", e);
      }
      return null;
    }
  ...
  export default function GarageList({
    ...
    const [minAvailabilityPct, setMinAvailabilityPct] = React.useState<number>(0);
    ...
    try {
      ...
      if (status !== "granted") {
        console.log("Location permission denied — proximity filter unavailable");
        return;
      }
      ...
      setOriginSource("gps");
      console.log("Using GPS location:", position.coords.latitude, position.coords.longitude);
    } catch (err) {
  ...
  const loadOrigin = React.useCallback(async () => {
    try {
      ...
      if (loadedOrigin) {
        const coords = await geocodeOriginAddress(loadedOrigin);
        if (coords) {
          setOriginCoords(coords);
          setOriginSource("saved");
          console.log("Geocoded saved origin:", coords);
          return;
        }
      }

      // No saved origin or geocoding failed — fall back to device GPS
      await loadCurrentLocation();
    } catch (err) {
      console.error("Failed to load starting location:", err);
      // API call failed — still try GPS
      await loadCurrentLocation();
    }
  }, [loadCurrentLocation]);

  // *** Call loadOrigin on mount ***
  React.useEffect(() => {
    loadOrigin();
  }, [loadOrigin]);
    ...
    React.useEffect(() => {
      const loadFilters = async () => {
        try {
          ...
          if (parsed?.maxDistanceMiles !== undefined) {
          setMaxDistanceMiles(parsed.maxDistanceMiles);
          }
          if (typeof parsed?.minAvailabilityPct === "number") {
          setMinAvailabilityPct(parsed.minAvailabilityPct);
          }
        } catch (error) {
    ...
    React.useEffect(() => {
      ...
      persist();
    }, [selectedPasses, showFavoritesOnly, maxDistanceMiles, minAvailabilityPct]);
    ...
    const getDistanceToGarage = React.useCallback(
      (garage: Garage): number | null => {
        if (!originCoords || !garage.lat || !garage.lng) return null;
        return calculateDistance(originCoords.lat, originCoords.lng, garage.lat, garage.lng);
      },
      [originCoords]
    );
    ...
    const visibleGarages = React.useMemo(() => {
      ...
      // Proximity filter
    if (maxDistanceMiles !== null && originCoords) {
      filtered = filtered.filter((garage) => {
        if (!garage.lat || !garage.lng) return true; // keep garages without coords
        const dist = calculateDistance(originCoords.lat, originCoords.lng, garage.lat, garage.lng);
        return dist <= maxDistanceMiles;
      });
    }

    // Availability threshold filter
    if (minAvailabilityPct > 0) {
      filtered = filtered.filter((garage) => {
        const total = garage.total || 1;
        const available = garage.current;
        const pct = (available / total) * 100;
        return pct >= minAvailabilityPct;
      });
    }

    if (originCoords) {
      filtered = [...filtered].sort((a, b) => {
        const distA = (a.lat && a.lng) ? calculateDistance(originCoords.lat, originCoords.lng, a.lat, a.lng) : Infinity;
        const distB = (b.lat && b.lng) ? calculateDistance(originCoords.lat, originCoords.lng, b.lat, b.lng) : Infinity;
        return distA - distB;
      });
    }

      return filtered;
    }, [garages, trimmedQuery, selectedPasses, showFavoritesOnly, maxDistanceMiles, minAvailabilityPct, originCoords]);
    ...
    const getRatings = async (code: string) => {
      ...
      let count = 0;
      if (selectedPasses.length > 0) count++;
      if (maxDistanceMiles !== null) count++;
      if (minAvailabilityPct > 0) count++;
      return count;
    ...
    const renderItem = ({ item }: { item: Garage }) => {
      ...
      const dist = getDistanceToGarage(item);
          ...
          >
            ...
            {dist !== null && (
              <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                <Ionicons
                  name={originSource === "saved" ? "home-outline" : "location-outline"}
                  size={13}
                  color={secondaryText}
                />
    ...
    const favoritesOnlyActive =
      showFavoritesOnly && selectedPasses.length === 0 && trimmedQuery.length === 0 && maxDistanceMiles === null && minAvailabilityPct === 0;

    const hasActiveFilters =
      trimmedQuery.length > 0 || selectedPasses.length > 0 || showFavoritesOnly ||
      maxDistanceMiles !== null || minAvailabilityPct > 0;
          ...
          >
            ...
            <TouchableOpacity
              ...
              style={{
                ...
                alignItems: "center",
                justifyContent: "center",
                backgroundColor: activeFilterCount > 0 ? theme.primary : theme.mode === "dark" ? "#2a2d33" : "#e5e7eb",
                position: "relative",
              }}
              hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}
            >
                  ...
                  <View>
                    ...
                    {originSource === "gps" && (
                      <Text style={{ color: theme.textMuted, fontSize: 12, marginBottom: 8 }}>
                        Using your current location. Set a starting location in Settings for more accurate results.
                      </Text>
                    )}
                    {originSource === "saved" && (
                      <Text style={{ color: theme.textMuted, fontSize: 12, marginBottom: 8 }}>
                        From your saved starting location
                      </Text>
                    )}
                    {originSource === "none" && (
                      <Text style={{ color: theme.textMuted, fontSize: 12, marginBottom: 8 }}>
                        Enable location access or set a starting location in Settings
                      </Text>
                    )}
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
                      {DISTANCE_OPTIONS.map((opt) => {
                        const isSelected = maxDistanceMiles === opt.value;
                        const isDisabled = originSource === "none" && opt.value !== null;
                        return (
                          <TouchableOpacity
                            key={opt.label}
                            onPress={() => {
                              if (isDisabled) return;
                              setMaxDistanceMiles(opt.value);
                            }}
                            style={{
                              paddingVertical: 8,
                              paddingHorizontal: 14,
                  ...
                  <View>
                    ...
                    <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
                      {AVAILABILITY_OPTIONS.map((opt) => {
                        const isSelected = minAvailabilityPct === opt.value;
                        return (
                          <TouchableOpacity
                            key={opt.label}
                            onPress={() => setMinAvailabilityPct(opt.value)}
                            style={{
                              paddingVertical: 8,
                              paddingHorizontal: 14,
                              borderRadius: 999,
